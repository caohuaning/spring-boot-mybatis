$(document).ready(function() {
	...
	$('.btn-toggle-full').on('click', function() {
		if(!$('body').hasClass('layout-full')) {
	...
	$(window).on('load', function() {
		if($(window).innerWidth() < 1025) {
			$('.btn-toggle-full').find('.icon-arrows')
			.removeClass('icon-arrows-move-left')
			.addClass('icon-arrows-move-right');
		}

		// adjust right sidebar top position
		$('.right-sidebar').css('top', $('.nav-bar').innerHeight());

		// if page has content-menu, set top padding of main-content
		if($('.has-content-menu').length > 0) {
			$('.nav-bar + .main-content').css('padding-top', $('.nav-bar').innerHeight());
		}
	...
	$('.sidebar a[data-toggle="collapse"]').on('click', function() {
        ...
        if($('body').hasClass('layout-full')) {
            $('body').removeClass('layout-full');
            $('.btn-toggle-full .lnr').toggleClass('lnr-arrow-left-circle lnr-arrow-right-circle');
        }
	});
